...
class VoiceService {
  ...
  private async setupPeerConnection(rtcConfig?: RTCConfiguration) {
    ...
    this.peerConnection.onconnectionstatechange = () => {
      // Connection state changed - could be used for UI updates
    };
  ...
  private async handleIncomingOffer(data: SdpData) {
  ...
  private async handleIncomingAnswer(data: SdpData) {
    if (!this.peerConnection) return;
  ...
  private async handleIncomingIceCandidate(data: IceCandidateData) {
    if (!this.peerConnection) return;
    ...
    try {
      ...
      this.onCallError?.('Failed to add ICE candidate');
    }
  }
