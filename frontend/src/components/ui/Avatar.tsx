...
export const Avatar: React.FC<AvatarProps> = ({
  ...
  const sizeClassName = typeof size === 'number' ? '' : sizeMap[size];
  const numericSizeStyle = typeof size === 'number'
    ? {
        width: `${size}px`,
        height: `${size}px`,
  ...
  if (children) {
    return (
      <div
        className={cn('relative rounded-full flex-shrink-0 overflow-hidden bg-gray-200', sizeClassName, className)}
  ...
  if (src) {
    return (
      <div 
        className={cn('relative flex-shrink-0 overflow-hidden', sizeClassName, className)}
        style={{ borderRadius: '100%', aspectRatio: '1/1', ...numericSizeStyle }}
      >
        <Image 
