...
export class MatchesService {
  ...
  async submitMatchAction(
    ...
    if (crossLike) {
      ...
      // REAL-TIME: Notify both users of the match
      this.chatGateway.broadcastToUser(userId, 'match:new', { chatId: chat.id });
      this.chatGateway.broadcastToUser(matchId, 'match:new', { chatId: chat.id });
      this.chatGateway.broadcastUpdate(['matches', 'relationships'], `user:${userId}`);
      this.chatGateway.broadcastUpdate(['matches', 'relationships'], `user:${matchId}`);
    ...
    // REAL-TIME: Notify the target user of the like
    this.chatGateway.broadcastToUser(matchId, 'like:received', { senderId: userId });
    this.chatGateway.broadcastUpdate(['relationships', 'notifications'], `user:${matchId}`);
