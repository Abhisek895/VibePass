...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  // ── GLOBAL REACTIVITY ENGINE (Industry Standard) ──
  
  /**
   * Broadcast a data update to specific rooms or globally
   * This triggers the frontend to invalidate its cache
   */
  async broadcastUpdate(keys: string[], roomId?: string) {
    if (!this.server) return; // Safety guard: Prevents crash during bootstrap
    
    const payload = { keys };
    if (roomId) {
      this.server.to(roomId).emit('data:invalidate', payload);
    } else {
      this.server.emit('data:invalidate', payload);
    }
  }
