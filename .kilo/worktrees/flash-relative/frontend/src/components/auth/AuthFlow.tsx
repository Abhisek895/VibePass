...
export default function AuthFlow({ initialView }: AuthFlowProps = {}) {
  ...
  const handleRequestOtp = async (context: OtpContext) => {
    if (!email) {
  ...
  const handleResetPassword = async (newPassword: string) => {
    ...
    try {
      ...
      const data = await apiRequest<any>('/api/v1/users/me', { auth: true });
      handleAuthSuccess({ accessToken: true, user: data }); // handleAuthSuccess uses accessToken from storage if not provided correctly, but here we just need to trigger the redirect
    } catch (error) {
  ...
  const handleResetUsername = async (newUsername: string) => {
    ...
    try {
      ...
      const { apiRequest } = await import('@/services/api/client');
      const data = await apiRequest<any>('/api/v1/users/me', { auth: true });
      handleAuthSuccess({ accessToken: true, user: data });
    } catch (error) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (view === 'register') {
      if (!ageConfirmed) {
        setErrorMsg('You must be 18-28 to join VibePass.');
        return;
      }

      void handleRequestOtp('signup');
      return;
    }
    ...
    login({ email, password })
      ...
      .catch((error) => {
        setErrorMsg(
          getApiErrorMessage(error, 'Network error. Is backend running?'),
        );
      })
          ...
          {(view === 'login' || view === 'register') && (
            <div className="mb-6 space-y-3">
              ...
              <p className="text-center text-xs text-[rgb(var(--text-muted))]">
                Social sign-in works locally now and can be connected to real
                provider OAuth keys later.
              </p>
            </div>
          )}

          <form onSubmit={handleSubmit} className="space-y-5">
            {view !== 'verify-otp' && view !== 'reset-password' && view !== 'reset-username' && (
            ...
            {view === 'register' && (
              <>
                ...
                <label className="group mt-4 flex cursor-pointer items-start space-x-3">
                  <div className="relative mt-0.5 flex h-5 w-5 items-center justify-center">
                    ...
                    />
                    <div className="h-5 w-5 rounded border border-[rgb(var(--text-muted))] transition-colors peer-checked:border-[rgb(var(--accent-primary))] peer-checked:bg-[rgb(var(--accent-primary))]" />
                    <svg
                  ...
                  <span className="text-sm text-[rgb(var(--text-secondary))] transition-colors group-hover:text-[rgb(var(--text-primary))]">
                    I confirm I am between 18 and 28 years old, and accept the
