import { Controller, Get, Param, Query, Post, Body, UseGuards, Request, NotFoundException, BadRequestException } from '@nestjs/common';
...
export class FacebookPostsController {
  ...
  async getUserPosts(
    ...
    try {
      ...
      // Get user's posts with eager loaded media
      const posts: any = await this.prisma.$queryRawUnsafe(`
        SELECT p.id, p.content, p.privacy, p.background_color, p.created_at, p.updated_at,
               p.reactions_count, p.comments_count, p.author_id,
  ...
  /**
   * GET /api/posts/feed
   * Get feed posts (all active posts, paginated)
   */
  @Get('feed')
  async getFeed(
    @Query('page') page: number = 1,
    ...
    try {
      ...
      const total: any = await this.prisma.$queryRawUnsafe(
        `SELECT COUNT(*) as count FROM facebook_clone.posts WHERE status = 'active'`
      );
      ...
      throw new NotFoundException('Failed to fetch feed');
    }
  }
  ...
  async createPost(
    @Body() { content, privacy }: { content: string; privacy?: string },
    @Request() req,
  ) {
    try {
      ...
      const result = await this.prisma.$queryRawUnsafe(`
        INSERT INTO facebook_clone.posts (author_id, content, privacy, status)
        VALUES (${req.user.id}, '${content.replace(/'/g, "\\'")}', '${privacy || 'public'}', 'active')
      `);
  ...
  async addComment(
    ...
    try {
      ...
      const result = await this.prisma.$queryRawUnsafe(`
        INSERT INTO facebook_clone.comments (post_id, author_id, content, status)
        VALUES (${postId}, ${req.user.id}, '${content.replace(/'/g, "\\'")}', 'active')
      `);
  ...
  async addReaction(
    ...
    try {
      ...
      return {
        ...
        reactionId: (result as any).insertId,
      };
    } catch (error) {
